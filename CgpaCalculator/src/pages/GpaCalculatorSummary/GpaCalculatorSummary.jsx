...
function getGpaLabel(gpa) {
  if (gpa >= 4.5) return { label: "Excellent", emoji: "🏆" };
  if (gpa >= 3.5) return { label: "Very Good", emoji: "🌟" };
  if (gpa >= 2.4) return { label: "Good", emoji: "👍" };
  if (gpa >= 1.5) return { label: "Pass", emoji: "📋" };
  return { label: "Below Pass", emoji: "⚠️" };
}
        ...
        return (
          <g key={g.grade}>
            ...
            <text x={x + barW / 2} y={y - 4} textAnchor="middle"
              style={{ fontSize: 8, fill: col, fontFamily: "Consolas, monospace", fontWeight: "bold" }}>
              {g.count}
            </text>
            {/* pct label inside bar (only if tall enough) */}
            {barH > 18 && (
              <text x={x + barW / 2} y={y + barH - 5} textAnchor="middle"
                style={{ fontSize: 7, fill: "#fff", fontFamily: "Consolas, monospace", opacity: 0.8 }}>
                {pct}%
              </text>
            )}
            {/* grade label below */}
            <text x={x + barW / 2} y={padT + innerH + 14} textAnchor="middle"
              style={{ fontSize: 9, fill: col, fontFamily: "Consolas, monospace", fontWeight: "bold" }}>
              {g.grade}
            </text>
            {/* pts label */}
            <text x={x + barW / 2} y={padT + innerH + 24} textAnchor="middle"
              style={{ fontSize: 7, fill: "var(--textColor)", opacity: 0.4, fontFamily: "Consolas, monospace" }}>
              {g.points}pt
            </text>
          </g>
        );
...
function GpaCalculatorSummary() {
  ...
  const gpaLabel = gpaDisplay !== null ? getGpaLabel(gpaDisplay) : null;

  // weighted units per grade
  const gradeUnits = gradeBreakdown.map((g) => ({
    ...g,
    units: courses
      .filter((c) => c.grade === g.grade)
      .reduce((sum, c) => sum + (parseFloat(c.unit) || 0), 0),
  }));

  const highestGrade = gradeBreakdown[0]?.grade ?? null;
  const lowestGrade = gradeBreakdown[gradeBreakdown.length - 1]?.grade ?? null;
        ...
        <main className={styles.mainContent}>
          ...
          {!hasData ? (
            <div className={styles.emptyState}>
              <span className="material-icons" style={{ fontSize: "4rem", color: "var(--accentGreen)", opacity: 0.35 }}>
                pending_actions
              </span>
              <h3 className={styles.emptyTitle}>Nothing to show yet</h3>
              <p className={styles.emptySub}>
                Go to the GPA Calculator, add your courses with units, and your summary will appear here.
              </p>
              <button className={styles.goBackBtn} onClick={() => navigate("/gpaCalculator")}>
                Go to Calculator
              </button>
            </div>
          ) : (
            <>
              ...
              <div
                ...
                style={{ background: `${gpaColor}15`, borderColor: `${gpaColor}40` }}
              >
                <span className={styles.heroEmoji}>{gpaLabel?.emoji ?? "📊"}</span>
                <div className={styles.heroMeta}>
                  ...
                  {gpaLabel && (
                    <div className={styles.heroClassLabel} style={{ color: gpaColor }}>
                      {gpaLabel.label}
                    </div>
                  )}
                </div>
              ...
              <div className={styles.statsGrid}>
                {[
                  ...
                  { label: "HIGHEST GRADE", value: highestGrade ?? "—", icon: "trending_up" },
                  { label: "LOWEST GRADE", value: lowestGrade ?? "—", icon: "trending_down" },
                    ...
                    return (
                      <div key={grade} className={styles.breakdownRow}>
                        <div
                          ...
                          style={{ background: `${gradeCol}20`, border: `1px solid ${gradeCol}50`, color: gradeCol }}
                        >
                          {grade}
                        </div>
                        <div className={styles.breakdownMeta}>
                          <div className={styles.breakdownTop}>
                            <span className={styles.breakdownInfo}>
                              {count} course{count !== 1 ? "s" : ""} · {unitSum} units · {points} pts each
                            </span>
                    ...
                    return (
                      <div key={course.id} className={styles.courseRow}>
                        ...
                        <div className={styles.courseCode}>
                          {course.code || <span style={{ opacity: 0.3 }}>—</span>}
                        </div>
