    ...
    <svg viewBox={`0 0 ${chartW} ${chartH}`} style={{ width: "100%", height: "auto", display: "block" }}>
      <defs>
        <linearGradient id="trajGrad" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#4caf7d" stopOpacity="0.3" />
      ...
      {thresholds.map((t) => (
        <g key={t.label}>
          <line x1={padL} y1={yPos(t.value)} x2={chartW - padR} y2={yPos(t.value)}
            stroke={t.color} strokeWidth="1" strokeDasharray="4 3" />
          <text x={padL - 4} y={yPos(t.value) + 3.5} textAnchor="end"
            style={{ fontSize: 8, fill: t.color.replace("55", "bb"), fontFamily: "Consolas, monospace" }}>
            {t.label}
          </text>
        </g>
      ))}
      ...
      {n >= 2 && (
        <>
          <polygon
            ...
            fill="url(#trajGrad)" />
          <polyline points={pts} fill="none" stroke="#4caf7d"
      ...
      {dataPoints.map((d, i) => (
        <g key={i}>
          <circle cx={xPos(i)} cy={yPos(d.value)} r={5}
            fill={d.color} stroke="var(--bg)" strokeWidth="2" />
          <text x={xPos(i)} y={yPos(d.value) - 9} textAnchor="middle"
            style={{ fontSize: 8.5, fill: d.color, fontFamily: "Consolas, monospace", fontWeight: "bold" }}>
            {d.value.toFixed(2)}
          </text>
          <text x={xPos(i)} y={padT + innerH + 16} textAnchor="middle"
            style={{ fontSize: 8, fill: "var(--textColor)", opacity: 0.5, fontFamily: "Consolas, monospace" }}>
            {d.label}
          </text>
        </g>
      ))}
          ...
          {!hasData ? (
            <div className={styles.emptyState}>
              ...
              <p className={styles.emptySub}>
                Go to the CGPA Predictor, fill in your current CGPA, total units, add your future courses and hit Predict CGPA — then come back here.
              </p>
              <button className={styles.goBackBtn} onClick={() => navigate("/cgpaPredictor")}>
                Go to Predictor
              </button>
            </div>
          ) : (
            <>
              ...
              <div
                ...
                style={{ background: `${predictedColor}15`, borderColor: `${predictedColor}40` }}
              >
                ...
                <div className={styles.heroMeta}>
                  ...
                  {predictedHonours && (
                    <div className={styles.heroHonours} style={{ color: predictedColor }}>
              ...
              <div className={styles.statsRow3}>
                ...
                <div className={styles.statCard}>
                  <div className={styles.statValue} style={{ color: predictedColor }}>
                    {predictedCgpa.toFixed(2)}
                  </div>
              ...
              {currentHonours && predictedHonours && currentHonours.label !== predictedHonours.label && (
                <div className={styles.classChange} style={{ background: `${predictedColor}0d`, borderColor: `${predictedColor}30` }}>
                  <span style={{ fontFamily: "Consolas, monospace", fontSize: "0.72rem", color: getGpaColor(currentVal), fontWeight: 700 }}>
                    {currentHonours.emoji} {currentHonours.label}
                  </span>
                  <span className="material-icons" style={{ fontSize: "1rem", color: "var(--accentGreen)", opacity: 0.7 }}>
                    arrow_forward
                  </span>
                  <span style={{ fontFamily: "Consolas, monospace", fontSize: "0.72rem", color: predictedColor, fontWeight: 700 }}>
